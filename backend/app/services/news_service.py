...
_rss_cache: dict[str, tuple[datetime, list[dict]]] = {}
_rss_cache_lock = _Lock()
_RSS_CACHE_TTL = timedelta(minutes=10)

_INDIAN_RSS_FEEDS: list[tuple[str, str]] = [
    ...
    ("https://www.thehindubusinessline.com/feeder/default.rss", "Business Line"),
    ("https://www.livemint.com/rss/news", "Mint"),
]


class IndianRssAdapter(NewsAdapter):
    """Fetches from major Indian financial RSS feeds and matches to the target stock."""
    provider = "indian_rss"
    ...
    def _fetch_feed(self, url: str, source: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("IndianRss feed error %s: %s", url, exc)
            return []
        items = []
        for item in root.findall(".//item"):
            items.append({
                ...
                "source": item.findtext("source") or source,
            })
        with _rss_cache_lock:
            _rss_cache[url] = (now, items)
        return items

    def _matches_stock(self, text: str, stock: Stock) -> bool:
        norm = normalize_text(text)
        company = normalize_text(stock.company_name or "")
        sym = (stock.symbol or "").lower()
        if len(company) >= 4 and company in norm:
            return True
        if len(sym) >= 3 and re.search(r"(?:^|\s)" + re.escape(sym) + r"(?:$|\s)", norm):
            return True
        return False

    def fetch_for_stock(self, stock: Stock, *, limit: int, published_after: datetime | None = None) -> ProviderFetchResult:
        articles: list[RawArticle] = []
        for feed_url, default_source in _INDIAN_RSS_FEEDS:
            for item in self._fetch_feed(feed_url, default_source):
                title = item["title"]
                link = item["link"]
                if not title or not link:
                    continue
                published_at = _parse_datetime(item["pubDate"]) or datetime.now(UTC)
                if published_after and published_at <= published_after:
                    continue
                if not self._matches_stock(f"{title} {item['description']}", stock):
                    continue
...
class FinnhubAdapter(NewsAdapter):
    """Finnhub company news — free tier: 60 req/min, ~30 days history per stock."""
    provider = "finnhub"
    ...
    def fetch_for_stock(self, stock: Stock, *, limit: int, published_after: datetime | None = None) -> ProviderFetchResult:
        sym = stock.symbol or ""
        exch = (stock.exchange or "NSE").upper()
        finnhub_symbol = f"{exch}:{sym}" if exch in ("NSE", "BSE") else sym
        from_dt = (published_after or (datetime.now(UTC) - timedelta(days=30))).strftime("%Y-%m-%d")
        to_dt = datetime.now(UTC).strftime("%Y-%m-%d")
        try:
            resp = requests.get(
                "https://finnhub.io/api/v1/company-news",
                params={"symbol": finnhub_symbol, "from": from_dt, "to": to_dt, "token": self.api_key},
                headers=REQUEST_HEADERS,
                timeout=settings.news_request_timeout_seconds,
            )
            resp.raise_for_status()
        ...
        if not isinstance(payload, list):
            return ProviderFetchResult(provider=self.provider, articles=[])
        articles: list[RawArticle] = []
...
def _provider_daily_limit(provider: str) -> int:
    ...
    if provider == "finnhub":
        return settings.news_finnhub_daily_limit
    return 0
...
def _adapters(db: Session) -> list[NewsAdapter]:
    ...
    adapters.append(IndianRssAdapter())
    adapters.append(GdeltAdapter())
    if settings.finnhub_api_key and _quota_available(
        db, "finnhub", settings.news_finnhub_daily_limit
    ):
        adapters.append(FinnhubAdapter(settings.finnhub_api_key))
    return adapters
...
def bulk_sync_news(
    db: Session,
    *,
    limit_stocks: int = 500,
    force: bool = False,
) -> dict[str, Any]:
    """Sync news for all active stocks — best-effort, skips fresh ones unless force=True."""
    stocks = list(
        db.scalars(
            select(Stock)
            .where(Stock.is_active.is_(True), Stock.is_bhav_index.is_(True))
            .order_by(Stock.symbol.asc())
            .limit(max(1, min(limit_stocks, 5000)))
        )
    )
    total = len(stocks)
    synced = 0
    ...
    failed = 0
    links_new = 0
    logger.info("bulk_sync_news START total=%d force=%s", total, force)
    for idx, stock in enumerate(stocks, 1):
        try:
            result = refresh_stock_news(db, stock.id, force=force, mode="bulk")
            status = result.get("status")
            new = int(result.get("links_new") or 0)
            if status == "skipped":
                skipped += 1
            else:
                synced += 1
            links_new += new
            if idx % 25 == 0 or new > 0:
                logger.info(
                    "bulk_sync_news [%d/%d] symbol=%s status=%s links_new=%d total_links=%d",
                    idx, total, stock.symbol, status, new, links_new,
                )
        except Exception:
            db.rollback()
            logger.warning("bulk_sync_news [%d/%d] FAILED symbol=%s", idx, total, stock.symbol)
            failed += 1
    logger.info(
        "bulk_sync_news DONE synced=%d skipped=%d failed=%d links_new=%d",
        synced, skipped, failed, links_new,
    )
    return {
        "status": "success" if failed == 0 else "warning",
        "stocks_selected": total,
        "synced": synced,
        "skipped": skipped,
        "failed": failed,
        "links_new": links_new,
...
def list_news_curator(
    ...
    *,
    ...
    """Paginated all-articles view with per-article stock tags for the News Curator page."""
    from sqlalchemy import desc

    page = max(1, page)
    per_page = min(max(per_page, 1), 100)

    query = select(StockNewsArticle)
    if provider:
        query = query.where(StockNewsArticle.provider == provider)
    if search:
        term = f"%{search.lower()}%"
        query = query.where(
            StockNewsArticle.headline.ilike(term) | StockNewsArticle.source.ilike(term)
        )

    total: int = int(db.scalar(select(func.count()).select_from(query.subquery())) or 0)
    articles = db.scalars(
        query.order_by(desc(StockNewsArticle.published_at)).offset((page - 1) * per_page).limit(per_page)
    ).all()

    # Batch-load stock tags for all fetched articles in one query
    from app.models.stock import Stock
    article_ids = [a.id for a in articles]
    tag_rows: list[Any] = []
    if article_ids:
        tag_rows = db.execute(
            select(StockNewsLink.article_id, Stock.symbol, Stock.yahoo_symbol, Stock.company_name)
            .join(Stock, Stock.id == StockNewsLink.stock_id)
            .where(StockNewsLink.article_id.in_(article_ids))
            .order_by(StockNewsLink.match_confidence.desc())
        ).all()
    ...
    items = [
        {
            **serialize_news_row(a),
            "stock_tags": tags_by_article.get(a.id, []),
        }
        for a in articles
    ]

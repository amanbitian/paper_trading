...
paired AS (
    SELECT
        ...
        r2.price_datetime AS previous_datetime
    FROM ranked r1
...
def _max_abs_daily_change_pct() -> float:
    return float(settings.market_movers_max_abs_daily_change_pct)


def _max_mover_candle_gap_days() -> int:
    return int(settings.market_movers_max_candle_gap_days)


def _requires_mover_volume() -> bool:
    return bool(settings.market_movers_require_volume)


def _quality_rules() -> dict[str, Any]:
    return {
        "max_abs_daily_change_pct": _max_abs_daily_change_pct(),
        "max_candle_gap_days": _max_mover_candle_gap_days(),
        "require_volume": _requires_mover_volume(),
    }


def _mover_quality_issue(row: dict[str, Any]) -> str | None:
    ...
    if _requires_mover_volume() and (volume is None or int(volume) <= 0):
        return "missing_or_zero_volume"
    if abs(change_pct) > _max_abs_daily_change_pct():
        return "extreme_daily_change"

    record_datetime = _as_datetime(row.get("record_datetime"))
    previous_datetime = _as_datetime(row.get("previous_datetime"))
    if record_datetime and previous_datetime:
        gap_days = (record_datetime.date() - previous_datetime.date()).days
        if gap_days > _max_mover_candle_gap_days():
            return "stale_previous_candle"
    return None


def _split_quality_rows(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    clean_rows: list[dict[str, Any]] = []
    issue_counts: dict[str, int] = {}
    for row in rows:
        issue = _mover_quality_issue(row)
        if issue:
            issue_counts[issue] = issue_counts.get(issue, 0) + 1
            continue
        clean_rows.append(row)
    return clean_rows, issue_counts


def _quote_quality_issue(quote: dict[str, Any]) -> str | None:
    try:
        price = _float(quote.get("price"))
        change_pct = _float(quote.get("change_pct"))
    except (TypeError, ValueError):
        return "invalid_quote_value"
    if price <= 0:
        return "non_positive_price"
...
def sanitize_market_movers_payload(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Remove impossible mover quotes from a cached/display payload.

    Fresh computations use row-level checks before rankings are built. Cached
    payloads may have been created before those checks existed, so this light
    display guard keeps old extreme quotes out of the UI without rewriting DB
    cache rows while ingestion is running.
    """
    if payload is None:
        return None

    cleaned = dict(payload)
    issue_counts = dict(cleaned.get("quality_issue_counts") or {})
    excluded = int(cleaned.get("quality_excluded_count") or 0)
    for key in ("top_gainers", "top_losers", "volume_shockers", "most_bought"):
        rows = list(cleaned.get(key) or [])
        kept: list[dict[str, Any]] = []
        for quote in rows:
            issue = _quote_quality_issue(quote)
            if issue:
                issue_counts[issue] = issue_counts.get(issue, 0) + 1
                excluded += 1
                continue
    ...
    if excluded:
        cleaned.setdefault("raw_eligible_count", cleaned.get("eligible_count", 0))
        cleaned["quality_excluded_count"] = excluded
        cleaned["quality_issue_counts"] = issue_counts
    cleaned["quality_rules"] = _quality_rules()
    return cleaned
...
def compute_market_movers_from_db(
    ...
    *,
    ...
    normalized_nifty_index = normalize_nifty_index_filter(nifty_index)
    raw_rows = compute_t1_mover_rows(db, nifty_index=normalized_nifty_index)
    rows, quality_issue_counts = _split_quality_rows(raw_rows)
    quality_excluded_count = sum(quality_issue_counts.values())
    if not rows:
        return {
            "record_date": None,
            "eligible_count": 0,
            "raw_eligible_count": len(raw_rows),

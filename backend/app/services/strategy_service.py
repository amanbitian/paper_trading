...
from app.services.fundamentals_service import get_quality_momentum_fundamental_context
...
QUALITY_MOMENTUM_TYPE = "quality_momentum"
...
def _strategy_price_lookback(parameters: dict, strategy_type: str) -> int:
    ...
    if strategy_type == QUALITY_MOMENTUM_TYPE:
        return max(
            int(parameters.get("momentum_long_window", 252))
            + int(parameters.get("momentum_skip_window", 21))
            + 1,
            int(parameters.get("trend_window", 200)),
            int(parameters.get("min_bars", 275)),
        ) + 5
...
def parameters_with_point_in_time_context(
    db: Session,
    strategy_type: str,
    parameters: dict,
    *,
    ...
    if strategy_type == QUALITY_MOMENTUM_TYPE and stock_id is not None:
        enriched.update(
            get_quality_momentum_fundamental_context(
                db,
                stock_id,
...
def generate_signal(db: Session, user: User, payload: GenerateSignalRequest) -> StrategySignal:
    ...
    parameters = parameters_with_point_in_time_context(
        db,
